...
def fetch_and_extract_latest_data():
    ...
    try:
        ...
        if all_latest_rows:
            ...
            # Store original result_df for display purposes
            display_df = result_df.copy()
            
            # Prepare data for database - select only necessary columns
            db_columns = ["trade_date", "company", "volume", "high", "low", "opening_price", "closing_price"]
            
            # Check if all required columns exist
            missing_cols = [col for col in db_columns if col not in result_df.columns]
            if missing_cols:
                print(f"⚠️  Missing columns for database: {missing_cols}")
                print(f"   Available columns: {list(result_df.columns)}")
                # Try to use what we have
                available_cols = [col for col in db_columns if col in result_df.columns]
                db_df = result_df[available_cols].copy()
            else:
                db_df = result_df[db_columns].copy()
            
            # Convert to date
            if 'trade_date' in db_df.columns:
                db_df["trade_date"] = pd.to_datetime(db_df["trade_date"]).dt.date
            
            # Rename columns
            if 'company' in db_df.columns:
                db_df.rename(columns={"company": "ticker"}, inplace=True)
            
            # Converting numeric columns
            num_cols = ["volume", "high", "low", "opening_price", "closing_price"]
            for col in num_cols:
                if col in db_df.columns:
                    db_df[col] = pd.to_numeric(db_df[col], errors="coerce")  # turns invalid values to NaN

            # Connect to sqlite3
            conn = sqlite3.connect("db/market_data.db")

            # Append to table
            db_df.to_sql("dse_tz_daily_ohlcv", conn, if_exists="append", index=False)
            ...
            print(f"\n{'='*60}")
            print(f"✅ SUCCESS! Saved {len(db_df)} latest records to database")
            ...
            # Show sample of saved data - using display_df which has all columns
            date_cols = [col for col in display_df.columns if 'date' in col.lower() and col != 'data_fetched_at']
            if date_cols:
                date_col = date_cols[0]
                print(f"\n📋 Sample of latest records:")
                for idx, row in display_df.head(3).iterrows():
                    # Check if source_url exists before accessing it
