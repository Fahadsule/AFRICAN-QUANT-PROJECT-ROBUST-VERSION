...
html_files = glob.glob("brvm_indices_*.html")

if not html_files:
    print("No brvm_indices_*.html files found in current directory")
    df = pd.DataFrame()
else:
    ...
    if all_dfs:
        ...
        #df.to_sql("brvm_indices_daily_ohlcv", engine, if_exists="append", index=False)
